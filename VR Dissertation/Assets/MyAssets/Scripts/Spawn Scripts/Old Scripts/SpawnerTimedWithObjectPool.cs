...
public class SpawnerTimedWithObjectPool : MonoBehaviour
{    
    ...
    IEnumerator WaitForSound()
    {
        while (audioSource.isPlaying == true)
        {
            if (timer > beat)
            {
                ...
                GameObject enemy = ObjectPooler.Instance.SpawnFromPool("Enemy", points[Random.Range(0, 4)], Quaternion.identity);
